...
pod_name=$( kubectl get pods -l name=jenkins --output=name | cut -f2 -d/)
...
kubectl cp $local_plugin_path  $remote_plugin_path
echo "Unzipping $plugin_full_path into $remote_plugin_directory" 
kubectl exec $pod_name -- unzip -o $plugin_full_path -d $remote_plugin_directory
